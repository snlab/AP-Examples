  ...
  protected Route onPacket(Packet p) {
    ...
    if (p.satisfies(Assertion.isARP())) {
      if (p.arpIpTarget() == vip) {
        ...
        sendPacket(frame, p.ingressPort());
      }
    }
    ...
    SwitchPort dstLoc = hostLocationMap.get(destHost);
    if (null == dstLoc) {
      return route(minSpanningTree(), edgePorts(), mods);
    }
    return route(shortestPath(p.ingressPort(), dstLoc), dstLoc, mods);
  }
